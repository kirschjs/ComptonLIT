...
C
C
      PARAMETER (NZOPER=11, NZOPQU=11, NZPARM=30, NZKMAX=80,
     *          NZUMAX=50, NZBMAX=39, NDIM=600, NDIM1=30000,
     *          NDIM2=9, NZFMAX=6, NZEKMA=100)
...
C
     ...
     *              LL1MAX(NZOPER-2)
C
      COMMON /OUTP/ EKNULL,EK(NZEKMA),INR,ISRB,JWSL,JWSR,MUL,NBAND2
C
...
C     1 = -  und  2 = +    only deltaPi is relevant
      READ(5,1002)JWSL,JWSR,NPARL,NPARR
...
C     BESTIMMUNG DER CLEBSCH DES RED. MATR.-EL.
      ...
      MUL2=2*MUL
      VF1=1.
      IF (MUL.GT.0) VF1=(AK+1.)/AK
      VF=8.*PI*VF1/(DF(MUL2+1)**2)
      VF=VF*HC/137.03604
      VF=VF*EKNULL
      VFE=VF1/(DF(MUL2+1)**2)
      
      CL=CLG(INT(2*GJR),INT(2*AK),INT(2*GJL),INT(2*GJR),INT(2*AKM))
C      CL=YG(GJR,AK,GJL,GJR,AKM)
      IF(CL.NE.0.) GOTO 101
      WRITE (6,1204) GJR,GJR,AK,AKM,GJL,GJL,CL
      STOP 4
C
C     PARITAETSCHECK
...
C      F=YG(GJR,AK,GJL,GJR,AKM)
      F=CLG(INT(2*GJR),INT(2*AK),INT(2*GJL),INT(2*GJR),INT(2*AKM))
...
C     NORM
      ...
      write(nout,*)'f6j:',LBL2,LBR2,0,JWSL,JWSR,ML
      write(nout,*) F6J(LBL2,LBR2,0,JWSL,JWSR,ML)
      FK2=FK1
...
C     ALLE OPERATOREN
      ...
      F2=F*FK2
C            1                                  Sp  Sn      
      GOTO(124,124,124,124,124,120,120,120,120,124,124,124,124,124,
     *                                         124,124,119), MKC
119   STOP 14
120   CONTINUE
C     BAHNOPERATOREN
...
C
C     ALLE OPERATOREN
C     FUER SPIN- UND SIEGERTOPERATOR FINDET KEINE MODIFIKATION VON
C     F1 BZW. F2 STATT. FUER DEN MAGN. SPINOPERATOR IST DAS RELATIVE
C     VORZEICHEN ZWISCHEN DURCHLAUF MIT MUL+1 UND MUL-1 BEREITS IN
C     LUISE BERUECKSICHTIGT.
C
124   CONTINUE
C     ALLE OPERATOREN
      F1=F1*GEFAK(MKC)
...
C
      ...
      CALL SORTEO(MKC,IH)
      IF(IGAK.GT.1) PRINT 1004,MKC,IH
C
...
C  generalize: CL -> (Jm_j Lm_L | J'm_J')
       OPWERT(NEK,MKC)=OPWERT(NEK,MKC)*SQRT(2.*GJL+1.)/CL/
     *             (ANORML*ANORMR)
...
C
C
C      ELEKTRISCHE OPERATOREN OHNE SIEGERTOPERATOR
       IF (MREG(2)+MREG(3)+MREG(6)+MREG(7).EQ.0) GOTO 640
       OPE=OPWERT(NEK,2)+OPWERT(NEK,3)+OPWERT(NEK,6)+OPWERT(NEK,7)
       IF (ISTEU.EQ.2) OPE=OPWERT(NEK,2)+OPWERT(NEK,3)+OPWERT(NEK,6)
       OPE=OPE*SQRT(VFE)
       IF (KMETH(1).GT.0) CALL OUT(NEK,1,OPE,1)
       IF (KMETH(2).GT.0) CALL OUT(NEK,1,(OPE/SQRT(VFUE)),2)
       FTRANS=(OPE*OPE)/FLOAT(JWSR+1)
       IF (KMETH(3).GT.0) CALL OUT(NEK,1,FTRANS,3)
       IF (KMETH(4).GT.0) CALL OUT(NEK,1,(FTRANS/VFUE),4)
C
C      ELEKTRISCHE OPERATOREN OHNE SIEGERTOPERATOR MIT KORREKTUR
       IF (ISTEU.GT.2) GOTO 640
       OPE=OPWERT(NEK,2)*GMP+OPWERT(NEK,3)*GMN+OPWERT(NEK,6)*GEP+
     *     OPWERT(NEK,7)*GEN
       OPE=OPE*SQRT(VFE)
       IF (KMETH(1).GT.0) CALL OUT(NEK,10,OPE,1)
       IF (KMETH(2).GT.0) CALL OUT(NEK,10,(OPE/SQRT(VFUE)),2)
       FTRANS=(OPE*OPE)/FLOAT(JWSR+1)
       IF (KMETH(3).GT.0) CALL OUT(NEK,10,FTRANS,3)
       IF (KMETH(4).GT.0) CALL OUT(NEK,10,(FTRANS/VFUE),4)
C
C
640    CONTINUE
C      ELEKTRISCHE OPERATOREN MIT SIEGERTOPERATOR
       IF (MREG(10)+MREG(11).EQ.0) GOTO 645
       OPE=OPWERT(NEK,2)+OPWERT(NEK,3)+OPWERT(NEK,10)+OPWERT(NEK,11)
       IF (ISTEU.EQ.2) OPE=OPWERT(NEK,2)+OPWERT(NEK,3)+OPWERT(NEK,10)
       OPE=OPE*SQRT(VFE)
       IF (KMETH(1).GT.0) CALL OUT(NEK,1,OPE,5)
       IF (KMETH(2).GT.0) CALL OUT(NEK,1,(OPE/SQRT(VFUE)),6)
       FTRANS=(OPE*OPE)/FLOAT(JWSR+1)
       IF (KMETH(3).GT.0) CALL OUT(NEK,1,FTRANS,7)
       IF (KMETH(4).GT.0) CALL OUT(NEK,1,(FTRANS/VFUE),8)
C      ECCE: LIT benchmark => no other operators or corrections
C            goto next energy
       GOTO 900
C
C      ELEKTRISCHE OPERATOREN MIT SIEGERTOPERATOR UND KORREKTUR
       IF (ISTEU.GT.2) GOTO 650
       OPE=OPWERT(NEK,2)*GMP+OPWERT(NEK,3)*GMN+OPWERT(NEK,10)*GEP+
     *     OPWERT(NEK,11)*GEN
C       OPE=OPE*SQRT(VFE)
       IF (KMETH(1).GT.0) CALL OUT(NEK,10,OPE,5)
       IF (KMETH(2).GT.0) CALL OUT(NEK,10,(OPE/SQRT(VFUE)),6)
       FTRANS=(OPE*OPE)/FLOAT(JWSR+1)
       IF (KMETH(3).GT.0) CALL OUT(NEK,10,FTRANS,7)
       IF (KMETH(4).GT.0) CALL OUT(NEK,10,(FTRANS/VFUE),8)
C
C
645    CONTINUE
C      ELEKTRISCHE OPERATOREN MIT MESONOPERATOREN
       IF (MREG(13)+MREG(14).EQ.0) GOTO 650
       OPE=OPWERT(NEK,2)+OPWERT(NEK,3)+OPWERT(NEK,6)+OPWERT(NEK,7)+
     *     OPWERT(NEK,13)+OPWERT(NEK,14)
       IF (ISTEU.EQ.2) OPE=OPWERT(NEK,2)+OPWERT(NEK,3)+OPWERT(NEK,6)+
     *     OPWERT(NEK,13)+OPWERT(NEK,14)
       OPE=OPE*SQRT(VFE)
       IF (KMETH(1).GT.0) CALL OUT(NEK,1,OPE,9)
       IF (KMETH(2).GT.0) CALL OUT(NEK,1,(OPE/SQRT(VFUE)),10)
       FTRANS=(OPE*OPE)/FLOAT(JWSR+1)
       IF (KMETH(3).GT.0) CALL OUT(NEK,1,FTRANS,11)
       IF (KMETH(4).GT.0) CALL OUT(NEK,1,(FTRANS/VFUE),12)
C
C      ELEKTRISCHE OPERATOREN MIT MESONOPERATOREN UND KORREKTUR
       IF (ISTEU.GT.2) GOTO 650
       OPE=OPWERT(NEK,2)*GMP+OPWERT(NEK,3)*GMN+OPWERT(NEK,6)*GEP+
     *     OPWERT(NEK,7)*GEN+OPWERT(NEK,13)+OPWERT(NEK,14)
       OPE=OPE*SQRT(VFE)
       IF (KMETH(1).GT.0) CALL OUT(NEK,10,OPE,9)
       IF (KMETH(2).GT.0) CALL OUT(NEK,10,(OPE/SQRT(VFUE)),10)
       FTRANS=(OPE*OPE)/FLOAT(JWSR+1)
       IF (KMETH(3).GT.0) CALL OUT(NEK,10,FTRANS,11)
       IF (KMETH(4).GT.0) CALL OUT(NEK,10,(FTRANS/VFUE),12)
C
C
650    CONTINUE
C      MAGNETISCHE OPERATOREN
       IF (MREG(4)+MREG(5)+MREG(8)+MREG(9).EQ.0) GOTO 655
       OPM=OPWERT(NEK,4)+OPWERT(NEK,5)+OPWERT(NEK,8)+OPWERT(NEK,9)
       IF (ISTEU.EQ.2) OPM=OPWERT(NEK,4)+OPWERT(NEK,5)+OPWERT(NEK,8)
       OPM=OPM*SQRT(VFE)
       IF (KMETH(1).GT.0) CALL OUT(NEK,2,OPM,1)
       IF (KMETH(2).GT.0) CALL OUT(NEK,2,(OPM/SQRT(VFUE)),2)
       FTRANS=(OPM*OPM)/FLOAT(JWSR+1)
       IF (KMETH(3).GT.0) CALL OUT(NEK,2,FTRANS,3)
       IF (KMETH(4).GT.0) CALL OUT(NEK,2,(FTRANS/VFUE),4)
C
C      MAGNETISCHE OPERATOREN
       IF (ISTEU.GT.2) GOTO 655
       OPM=OPWERT(NEK,4)*GMP+OPWERT(NEK,5)*GMN+OPWERT(NEK,8)*GEP+
     *     OPWERT(NEK,9)*GEN
       OPM=OPM*SQRT(VFE)
       IF (KMETH(1).GT.0) CALL OUT(NEK,20,OPM,1)
       IF (KMETH(2).GT.0) CALL OUT(NEK,20,(OPM/SQRT(VFUE)),2)
       FTRANS=(OPM*OPM)/FLOAT(JWSR+1)
       IF (KMETH(3).GT.0) CALL OUT(NEK,20,FTRANS,3)
       IF (KMETH(4).GT.0) CALL OUT(NEK,20,(FTRANS/VFUE),4)
C
C
655    CONTINUE
C      MAGNETISCHE OPERATOREN MIT MESONOPERATOREN
       IF (MREG(15)+MREG(16).EQ.0) GOTO 660
       OPM=OPWERT(NEK,4)+OPWERT(NEK,5)+OPWERT(NEK,8)+OPWERT(NEK,9)+
     *     OPWERT(NEK,15)+OPWERT(NEK,16)
       IF (ISTEU.EQ.2) OPM=OPWERT(NEK,4)+OPWERT(NEK,5)+OPWERT(NEK,8)+
     *                 OPWERT(NEK,15)+OPWERT(NEK,16)
       OPM=OPM*SQRT(VFE)
       IF (KMETH(1).GT.0) CALL OUT(NEK,2,OPM,9)
       IF (KMETH(2).GT.0) CALL OUT(NEK,2,(OPM/SQRT(VFUE)),10)
       FTRANS=(OPM*OPM)/FLOAT(JWSR+1)
       IF (KMETH(3).GT.0) CALL OUT(NEK,2,FTRANS,11)
       IF (KMETH(4).GT.0) CALL OUT(NEK,2,(FTRANS/VFUE),12)
C
C      MAGNETISCHE OPERATOREN MIT MESONOPERATOREN UND KORREKTUR
       IF (ISTEU.GT.2) GOTO 660
       OPM=OPWERT(NEK,4)*GMP+OPWERT(NEK,5)*GMN+OPWERT(NEK,8)*GEP+
     *     OPWERT(NEK,9)*GEN+OPWERT(NEK,15)+OPWERT(NEK,16)
       OPM=OPM*SQRT(VFE)
       IF (KMETH(1).GT.0) CALL OUT(NEK,20,OPM,9)
       IF (KMETH(2).GT.0) CALL OUT(NEK,20,(OPM/SQRT(VFUE)),10)
       FTRANS=(OPM*OPM)/FLOAT(JWSR+1)
       IF (KMETH(3).GT.0) CALL OUT(NEK,20,FTRANS,11)
       IF (KMETH(4).GT.0) CALL OUT(NEK,20,(FTRANS/VFUE),12)
C
660    CONTINUE
C      COULOMBOPERATOR
       IF (MREG(NZOPER-1)+MREG(NZOPER).EQ.0) GOTO 900
       OPC=OPWERT(NEK,NZOPER-1)+OPWERT(NEK,NZOPER)
       IF (ISTEU.EQ.2) OPC=OPWERT(NEK,NZOPER-1)
       OPC=OPC*SQRT(VFE/VF1)
       IF (KMETH(1).GT.0) CALL OUT(NEK,3,OPC,1)
       IF (KMETH(2).GT.0) CALL OUT(NEK,3,(OPC/SQRT(VFUE)),2)
       FLONG=(OPC*OPC)/FLOAT(JWSR+1)
       IF (KMETH(3).GT.0) CALL OUT(NEK,3,FLONG,3)
       IF (KMETH(4).GT.0) CALL OUT(NEK,3,(FLONG/VFUE),4)
C
C      KORREKTUR DES FORMFAKTORS UM LADUNGSFORMAFAKTOR VON PROTON
C      UND NEUTRON
       IF (ISTEU.GT.2) GOTO 900
       OPC=OPWERT(NEK,NZOPER-1)*GEP+OPWERT(NEK,NZOPER)*GEN
       OPC=OPC*SQRT(VFE/VF1)
       IF (KMETH(1).GT.0) CALL OUT(NEK,30,OPC,1)
       IF (KMETH(2).GT.0) CALL OUT(NEK,30,(OPC/SQRT(VFUE)),2)
       FLONG=(OPC*OPC)/FLOAT(JWSR+1)
       IF (KMETH(3).GT.0) CALL OUT(NEK,30,FLONG,3)
       IF (KMETH(4).GT.0) CALL OUT(NEK,30,(FLONG/VFUE),4)
C
C
900   CONTINUE
      GOTO 980
...
C     ELEKTISCHE OPERATOREN
      IF(MREG(2)+MREG(3)+MREG(6)+MREG(7)+MREG(10)+MREG(11)+
     *    MREG(13)+MREG(14).EQ.0) GOTO 970
      WRITE (6,1053)
      WRITE (6,1054) MUL
      ...
      WRITE (6,1057) MUL
      IF (MREG(13)*MREG(14).NE.0) THEN
         OPWE(3)=OPWERT(1,12)+OPWERT(1,13)
         WRITE (6,1039) OPWE(3), MUL
      ELSE
         OPWE(3)=0.
         WRITE (6,1090)
...
970   CONTINUE
C     MAGNETISCHE OPERATOREN
      IF(MREG(4)+MREG(5)+MREG(8)+MREG(9)+MREG(15)+MREG(16).EQ.0)
     *                                                  GOTO 980
      WRITE (6,1052)
      WRITE (6,1058) MUL
      ...
      WRITE (6,1060) MUL
      IF (MREG(15)*MREG(16).NE.0) THEN
         OPWE(2)=OPWERT(1,15)+OPWERT(1,16)
         WRITE (6,1039) OPWE(2), MUL
      ELSE
         OPWE(2)=0.
         WRITE(6,1090)
...
1036  FORMAT(/,1X,'ENERGIE  DES PHOTONS:',F12.6,' MEV')
...
1300  FORMAT(I10,' ELEMENTE FUER WEITE RECHTS,LINKS,DC,OPERATOR',
     * ',ZERLEGUNG RECHTS,LINKS ',6I5,' GESCHRIEBEN')
      ...
      PARAMETER (NDIM=600, NDIM1=30000, NDIM2=9, NZOPER=11)
C
      COMMON /CSOR/ MMM, NTI, OP(NDIM2,2,NDIM,NDIM),EFG(2,NDIM,NDIM),
     *              ECFG(NDIM1,NZOPER-2), OCP(NDIM2,NDIM1,NZOPER-2),
     *              LL1MAX(NZOPER-2)
...
C
      PARAMETER (NZEKMA=100)
C
      COMMON /OUTP/ EKNULL, EK(NZEKMA), INR, ISRB, JWSL, JWSR, MUL,
     *              NBAND
C
      EK2=EK(NEK)*EK(NEK)
      WRITE (6,1000) EK2, EK(NEK), MUL, INR, ISRB, IP, JWSL,
     *               JWSR, 0, OP, 0., MTD
      WRITE (NBAND,1000) EK2, EK(NEK), MUL, INR, ISRB, IP,
     *                   JWSL, JWSR, 0, OP, 0., MTD
      RETURN
1000  FORMAT (2E12.4,7I3,2E12.4,I3)
      END
