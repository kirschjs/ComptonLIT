...
C
C
      PARAMETER (NZOPER=11, NZOPOB=13, NZOPLU=7,  NZTMAX=12,
     *           NZFMAX=20, NZCMAX=4,  MZGMAX=9,  NZLWMA=14,
     *           NZRHOM=30, MZPARM=30, NZPARM=20, NZBMAX=80,
     *           NZPOMA=10, NZIQMA=7,  NDIM=600, NDIM1=4000,
     *           NDIM2=60, NDIM5=1600)
...
C
     ...
     *        VZ(NZVMAX,NZVMAX), WERT(NDIM4,NDIM4,NDIM2), LPARR, MC1,
     *        MD1, MFL, NCC, NREB, NZAV, NZV1, PHI2,
     *        RPAR(MZPARM,NZFMAX), VR(NZTMAX,NZCMAX), WR(NZTMAX),
     *        FG, LAMB(NDIM4*NDIM4), LAHI(NZBMAX*NZBMAX),
     *        NUHI(NZBMAX*NZBMAX)
C
C
...
C
C
      IF(I.GT.NZBMAX) WRITE(NOUT,1985)
...
6100  FORMAT(/31H ES WIRD VOELLIG NEU GERECHNET /)
      END
      SUBROUTINE LURE (KZAHL)
C     LURE LIEST DIE RECORDS AUS LUISE EIN
C
      PARAMETER (NZOPLU=7, NZCMAX=4, NZLWMA=14, NZPOMA=10,
     *           NZIQMA=7, NDIM1=4000)
C
...
C     AUS INDEX
C
      PARAMETER (NZCMAX=4, NZLWMA=14, NZPOMA=10, NZIQMA=7,
     *           NDIM1=4000)
C
      PARAMETER (NZAVMA=2*(NZCMAX-1), NDIM4=NZPOMA*NZLWMA)
C
...
C
      PARAMETER (NZTMAX=12, NZFMAX=20, NZCMAX=4, NZLWMA=14,
     *           MZPARM=30, NZBMAX=80, NZPOMA=10, NDIM=600,
     *           NDIM2=60, NDIM5=1600)
C
      PARAMETER (NZAVMA=2*(NZCMAX-1), NZVMAX=NZTMAX-1,
     *           NDIM4=NZPOMA*NZLWMA)
...
1003  FORMAT(1X,'NZAVZU =',I2)
      DO 44 N = 1,NZV
       PRINT 1002,(QO(N,M),M=1,NZV)
1002  FORMAT(1X,'QO(N,M)',(/,1X,8E12.4))
   44 CONTINUE
      PRINT 1005, (WR(M),M=1,NZV)
1005  FORMAT(1X,'WR(M)  :',8E12.4)
      DO 46, M=1, NZV
       PRINT 1006, M, (VZ(M,N), N=1, NZV)
46    CONTINUE
1006  FORMAT(1X,'VZ(',I2,',N)  :',8E12.4)
      DO 47, N=1, NZV
       PRINT 1007, N, (VR(N,M), M=1, MD1)
47    CONTINUE
...
C
      ...
      PRINT 1008, (OMEGA(M), M=1, NZV)
1008  FORMAT (1X,9HOMEGA'S :,8E12.4)
      PRINT 1009, (ALPHA(M), M=1, NZAV)
1009  FORMAT (1X,9HALPHA'S :,8E12.4)
      DO 48, N=1, NZV
       PRINT 1010, N, (P(M,N), M=1, NZAV)
48    CONTINUE
1010  FORMAT(1X,'P(M,',I2,') :',8E12.4)
      PRINT 1011, (S(I), I=1, (NZAV*(NZAV+1)/2))
...
C     ADREESRECHNUNG UND BESTIMMUNG VON DMM
      ...
      DO 100 M=1,NSH
       ...
       DO 100 K = 1,LL1
        ...
        write(nout,*) M,NSH,K,LL1,SH(M),WERTT(I2,K),DMM(I1,K),FG
        PRINT 50,M,NSH,K,LL1,SH(M),WERTT(I2,K),DMM(I1,K),FG
...
C
      PARAMETER (NZTMAX=12, NZFMAX=20, NZCMAX=4, NZLWMA=14,
     *           MZPARM=30, NZPOMA=10, NZIQMA=7, NDIM=600,
     *           NDIM1=4000, NDIM2=60)
C
      PARAMETER (NZAVMA=2*(NZCMAX-1), NZVMAX=NZTMAX-1,
     *           NDIM4=NZPOMA*NZLWMA)
...
60      M1 = KVK(K)
        ...
        PRINT 1002,K1ZAHL,K2ZAHL,K,MM,M1,KZOM,VORFAK,
...
1002  FORMAT(1X,'K1,K2,K,MM,M1,KZOM,FF,SIG,EPO,WERTT,LINT:',/,
     *       2I3,2I4,I5,I3,4E16.8,6I3)
