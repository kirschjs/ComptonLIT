...
The generalized sum rule of a 1-dimensional function

\begin{eqnarray*}
L(\alpha)&:=&\int^\infty_{e_0}\,\frac{r(e)}{(\alpha-e)^2+\beta^2}~de
=\int_{e_0}^\infty\,\frac{1}{(\alpha-e)^2+\beta^2}\cdot
\frac{1}{(\gamma-e)^2+\delta^2}~de\\
&\vdots&\\
&=&\left[(\alpha-\gamma)^2+(\beta-\delta)^2\right]^{-1}\cdot
\left[(\alpha-\gamma)^2+(\beta+\delta)^2\right]^{-1}\\
&&\cdot
\Bigg\lbrace
\beta^{-1}\left((\alpha-\gamma)^2+\delta^2-\beta^2\right)
\left(\frac{\pi}{2}+\tan^{-1}\left(\frac{\alpha\mprime}{\beta}\right)\right)\\
&&+~
\delta^{-1}\left((\alpha-\gamma)^2-\delta^2+\beta^2\right)
\left(\frac{\pi}{2}+\tan^{-1}\left(\frac{\gamma\mprime}{\delta}\right)\right)\\&&+~
(\alpha-\gamma)~\ln\left(\frac{\alpha\mprime^2+\beta^2}{\gamma\mprime^2+\delta^2}
\right)
\Bigg\rbrace\;\;,
\end{eqnarray*}
with $x\mprime:=x-e_0$.
