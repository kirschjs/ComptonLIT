...
$$S^{Jm_jJ_0}_{rs}:=\me{\phi^{J^\pi m_j}_r}{\hat{\mathcal{O}}_{Lm_L}}{\phi^{J_0^{\pi_0}J_0}_s}\;\;,$$
and superimpose these matrix elements according to Eq.\eqref{eq.liteq.mat}
\be\la{eq.litinhomo}
S^{Jm_j}_r:=\sum\limits_{m_L}~\clg{L}{J_0}{m_L}{\red{m_j}\black{-m_L}}{J}{m_j}~\underbrace{\sum\limits_{n}^{N_0}~c_n~S^{Jm_jJ_0}_{rn}}
