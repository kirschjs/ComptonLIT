...
while ((abs(EBDG + 2.22) > 0.21) & (ite < 1)):
    ...
    for streukanal in streukas:
        ...
        seedwidths1 = np.linspace(0.0005, 1.1, 25)
        seedwidths2 = np.linspace(0.0001, 1.1, 25)
        ...
        for frg in range(len(lfrags)):
            ...
            #seedwidths2tmp = np.array(seedwidths2) + np.random.randn(
            #) * np.array(seedwidths2) / widthcluster
            ...
            #rho2widths = [
            #    wcand for wcand in np.sort(
            #        np.abs(seedwidths2 + seedwidths2tmp))[::-1] if wcand < upth
            #]
            rho1widths = np.sort(np.abs(seedwidths1))[::-1]
            rho2widths = np.sort(np.abs(seedwidths2))[::-1]
